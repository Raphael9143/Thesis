...
export default function CreateCourseModal({ open, onClose, defaultClassId = null, onCreated }) {
	...
	const onSubmit = async (e) => {
		...
		if (!courseName || !courseCode || !defaultClassId) {
			...
			setNotifyType('error'); setNotifyOpen(true); return;
		}
		setSubmitting(true);
		try {
			const payload = {
			...
			if (res?.success) {
				setNotifyMsg('Course created'); setNotifyType('success'); setNotifyOpen(true);
				onCreated?.(res.data);
				onClose?.();
			} else {
				setNotifyMsg(res?.message || 'Failed to create course'); setNotifyType('error'); setNotifyOpen(true);
			}
		} catch (err) {
			const msg = err?.response?.data?.message || err.message || 'Server error';
			setNotifyMsg(msg); setNotifyType('error'); setNotifyOpen(true);
		} finally {
			setSubmitting(false);
		}
	};
		...
		<Modal open={open} onClose={onClose} title="Create Course">
			<form onSubmit={onSubmit} className="create-class-modal-form">
				<div className="create-class-row">
					<FormField label="Course name" value={courseName} onChange={e => setCourseName(e.target.value)} required={true} />
					<FormField label="Course code" value={courseCode} onChange={e => setCourseCode(e.target.value)} required={true} />
				</div>
				...
				<div className="create-class-actions">
					...
					<button type="button" className="btn btn-signin" onClick={() => { setCourseName(''); setCourseCode(''); setDescription(''); setSemester(''); }}>Reset</button>
					<button type="button" className="btn btn-signin" onClick={onClose}>Cancel</button>
				</div>
