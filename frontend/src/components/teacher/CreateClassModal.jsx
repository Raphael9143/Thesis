		...
		<Modal open={open} onClose={onClose} title="Create Class">
			<form onSubmit={onSubmit} className="create-class-modal-form">
				...
				<div className="create-class-student-emails">
					...
					{suggestions.length > 0 && (
						<div className="suggestions">
							{suggestions.map(s => (
								<button key={s} className="btn" onClick={() => addEmail(s)}>{s}</button>
							))}
						</div>
					)}
				...
				<div className="create-class-actions">
					...
					<button type="button" className="btn btn-signin" onClick={(e) => onSubmit(e, 'draft')}>Save Draft</button>
					<button type="button" className="btn btn-signin" onClick={() => { setName(''); setCode(''); setDescription(''); setSelectedStudentEmails([]); setMaxStudents(30); }}>Reset</button>
