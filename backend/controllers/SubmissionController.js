  ...
  submitAssignment: async (req, res) => {
    try {
      if (!req.user || req.user.role !== "STUDENT") {
        return res
          .status(403)
          .json({ success: false, message: "Chỉ sinh viên mới được nộp bài." });
      }
      ...
      if (!isMember)
        return res
          .status(403)
          .json({ success: false, message: "Bạn không thuộc lớp này." });
      ...
      if (assignment_id) {
        ...
        if (!assignment)
          return res
            .status(404)
            .json({ success: false, message: "Assignment không tồn tại." });
        ...
        if (!assignmentCourse)
          return res
            ...
            .json({
              success: false,
              message: "Bài tập này không thuộc lớp này.",
            });
        ...
        if (!exam)
          return res
            .status(404)
            .json({ success: false, message: "Exam không tồn tại." });
        // verify exam belongs to class via ClassCourse linking course to class
        const classCourse = (await AssignmentCourse.sequelize.models
          .ClassCourse)
          ? null
          : null;
        ...
        if (!link)
          return res
            ...
            .json({
              success: false,
              message: "Bài kiểm tra này không thuộc lớp này.",
            });
    ...
    try {
      if (!req.user || req.user.role !== "TEACHER") {
        return res
          ...
          .json({
            success: false,
            message: "Chỉ giáo viên mới được chấm điểm.",
          });
      }
